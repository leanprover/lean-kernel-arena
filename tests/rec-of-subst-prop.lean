...
`resultSort h` K-reduces to `Prop` for a variable `h : GateP`, so `Owner` is
accepted as a family of propositions whose constructor carries a `A : Type`
field, while `Owner gateWitness`, well-typed because `GateQ ≡ GateP`, has a sort
that stays stuck.

Rather than projecting that field out, the exploit eliminates `Owner` with its
own (`Prop`-only) recursor into `Fiber X := Acc emptyTypeRel X`, which is a
proposition, and recovers the data from there: `Acc.rec` eliminates `Acc` into
`Type`, and `propext` transports an `Acc` proof between two `Fiber` types.  Since
`Owner` is a proposition, proof irrelevance identifies `Owner.mk gateWitness
...
models; it is the test added in leanprover/lean4#14847.
-/
import Lean
...
private def eqNatExpr (left right : Expr) : Expr := eqExpr natExpr left right (Level.succ Level.zero)
private def reflNatExpr (value : Expr) : Expr := reflExpr natExpr value (Level.succ Level.zero)
...
private def rawDelayExpr (p value : Expr) : Expr :=
  ...
  mkAppN (mkConst ``Eq.rec [Level.zero, (Level.succ Level.zero)])
