...
Here the cheap component comes first; `refute-cheap-last.lean` swaps them.
...
run_elab do
  ...
  let num : Nat → Expr := fun k => Id.run do
    ...
    return e
  let prodTy := mkApp2 (mkConst ``Prod [0, 0]) bool ty
  let pair : Expr → Expr → Expr := fun a b =>
    mkApp4 (mkConst ``Prod.mk [0, 0]) bool ty a b
  let lhs := pair (mkConst ``Bool.false) (mkApp (mkConst ``count) (num n))
  let rhs := pair (mkConst ``Bool.true) (mkApp (mkConst ``count) (num (n + 1)))
