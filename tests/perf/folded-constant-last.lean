...
under a function that forces it. Here the plain occurrence comes first;
`folded-constant-first.lean` swaps them, and the ratio between the two files
is what a checker pays when the forced copy is reduced before the plain pair
is compared.
...
run_elab do
  ...
  Lean.addDecl (.thmDecl {
    name := `kernel_folded_constant_last
    levelParams := []
